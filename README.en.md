...
The Agent cites these as it answers, and you can verify every claim. For **content-type questions** such as "what does it say about X", the skill also requires the Agent to **expand instead of compress**: dates, numbers, proper nouns, conditions and exceptions from the hits are kept item by item — never lumped into one sentence or replaced with vague quantifiers. The Agent also **deep-reads each source section before answering** — a retrieval snippet preview never serves as the content. Every answer ends with a **"source links" list** (a hard requirement the skill puts on the Agent): documents come with a clickable link to the original file plus a page/chapter label, audio and video with a "▶ play from mm:ss" link — every source cited in that round of retrieval is listed, one click away. When the client does not render custom-protocol links, a copy-pasteable playback command is given alongside.
