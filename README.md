...
### 为 Agent 而设计：这些选择都是刻意的

> **设计初衷：Agent 有理解与表达，缺的是"资料可检索、结论可溯源、上下文可控"。MyAgentRAG 只补这一半**——并按 Agent 的实际使用方式设计每一处接口：

- **输出即契约**：stdout 只有 JSON（进度走 stderr，`--quiet` 可关）；退出码 0/1/2；错误带双语 `error_i18n`；删除类操作先返回 `confirm_required`——Agent 可直接分支处理，无需解析人类文案；
- **命中自带可点击出口与出处**：每条结果附 `locator`（文档打开原文件、媒体从该秒定位播放）与结构级出处（PDF 页码 / EPUB 章节 / 文本行号 / 音视频秒级时间戳），Agent 渲染即可用；
- **上下文预算优先**：命中列表是"索引"而非正文（默认最多 10 条、每条引用 1–2 行），精读按 `--section` / `--chunk` / `--max-chars` 分段，超长文档自动分片——物理上塞不爆上下文窗口；
- **取数与呈现有硬性契约**：内容型提问先逐处精读原文再作答，回答固定为"结论 + 命中列表 + 末尾源文链接清单"并带自检门禁——**换成任何 Agent、任何问法，输出的版式与出处都一致**；
- **不必猜"是不是拿全了"**：检索结果自带候选总数与截断标记，被截断时给出补救建议，内容型/枚举型提问自动加大取数深度；
- **技能文档也按 Agent 的加载方式设计**：精简主文件 + 按需引用（`references/`）、截断自诊断哨兵、`--contract` 兜底——文档即使只被加载了一部分，也不会"带着错误的假设干活"。
...
每条命中都带：所属条目与章节标题、原文摘要片段（『』高亮）、多路得分明细，以及**精确出处**。技能对 Agent 有统一的**结果呈现契约**（硬性要求）：命中列表按固定格式逐条呈现（编号 + **标题** + 出处标注 + 引用片段），回答末尾必附「源文链接」清单——无论你用哪个 Agent、怎么提问，检索结果的版式都是一致的。
...
Agent 引用这些出处作答，你可以逐条核实。对「讲了什么」这类**内容型提问**，技能还要求 Agent **展开呈现而不是压缩**：命中里的年代、数字、专名、条件与例外逐项保留，不得用“多个/若干”一笔带过，也不得把几个要点并成一句。而且 Agent 会**先逐处精读原文再作答**——检索返回的预览片段（snippet）不得充当内容。**每次回答的末尾都会附上一份「源文链接」清单**（技能对 Agent 的硬性要求）：文档给可点击的原文件链接 + 页码/章节标注，音视频给「▶ 从 mm:ss 播放」链接——同一次检索里引用过的来源逐条列出，点开即达；客户端不支持自定义协议链接时，并列给出可复制的播放命令。
...
命中结果还带一个可点击的 `locator`，**文档与媒体统一走技能自己的 `myagentrag://` 协议链接**（点击由已注册的协议处理器接管：媒体从命中时间戳起播，文档打开原文件并并列页码/章节标注）——不再依赖客户端对 `file://` 的处理策略，各阅读器不支持深链，故只承诺"打开原文件"，页码以文字并列给出。
...
**架构一句话**：Agent（理解意图 / 精读 / 作答）调用 `scripts/extract.py`（只负责提取与索引，不调用 LLM）→ 提取器按类型分流（网页与字幕、文档解析、音视频转录）→ 写入本地 **workspace**（单个 SQLite 文件承载全文索引、向量与标题锚点，目录自包含、拷走即迁移）→ 组件全部装在 `~/.myagentrag/`（专用运行时、ffmpeg、whisper、模型），删除即卸载。

它与其他"把文件丢给模型"的做法相比，四个特性最直接：

- **纯本地闭环**：提取、索引、检索、精读不出本机；不调用任何 LLM（无 key、无外部 API）。仅"网页正文抓取"会经 Jina Reader（URL 发送至第三方），用本地文件可完全绕开；
- **三路混合检索 + RRF 融合**：关键词（精确用词）、向量语义（换说法、跨语言）、标题锚点（直接命中章节）并行召回，同节聚合去重；
- **出处锚定到结构**：PDF 页码、EPUB 章节、文本行号、音视频秒级时间戳；命中自带可点击链接——文档打开原文件、媒体从该秒定位播放；
- **取数与呈现有硬性契约**：候选规模可感知（截断会提示并给补救建议），内容型提问先逐处精读原文再作答，回答末尾固定附「源文链接」清单——换成任何 Agent、任何问法，版式与出处都一致。

**入库流程**（横向：从用户一句话到可检索）：

![入库流程](images/ingest-flow.svg)

**检索与回答流程**（横向：三路召回 → 融合 → 精读 → 带出处作答）：

![检索与回答流程](images/retrieval-flow.svg)
...
从旧版本升级后若提示找不到 ffplay（早期装机可能只落了 ffmpeg），执行一次 `--repair-deps` 即可幂等补齐内置组件（缺谁补谁；已齐全则零下载、不改动已有文件）。
