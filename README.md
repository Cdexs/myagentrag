...
Agent 引用这些出处作答，你可以逐条核实。而且**每次回答的末尾都会附上一份「源文链接」清单**（技能对 Agent 的硬性要求）：文档给可点击的原文件链接 + 页码/章节标注，音视频给「▶ 从 mm:ss 播放」链接——同一次检索里引用过的来源逐条列出，点开即达；客户端不支持自定义协议链接时，并列给出可复制的播放命令。
