...
| "库中关于 X **讲了什么 / 说了哪些 / 具体内容是什么**" | **内容型** | ① `--search "X" --limit 50` 取候选；② 对**每一处**将呈现的命中 `--section` 精读（**不得只凭 `snippet` 作答**）；③ 按「详略档位 · 内容型」**展开**输出 + 末尾源文链接清单 |
