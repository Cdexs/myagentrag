...
与客户端、提问方式无关，结构固定：**检索轮次 = 检索概况 → 命中列表 → 源文链接清单**；**精读轮次（讨论指定章节/条目内容）= 内容或结论 → 源文链接清单**（无命中列表）。提问类问题先给结论，再附列表与清单；结论的行内引用用同一套标注。
...
3. **源文链接清单**（回答末尾必附，规则见下）；精读轮次无第 1、2 段，直接「内容/结论 → 清单」。
...
"$PYTHON" "$EXTRACTOR" --play-uri "myagentrag://play?ws=<库名>&entry=<id>&at=<秒>"   # 协议入口（点击链接时调用）
```
